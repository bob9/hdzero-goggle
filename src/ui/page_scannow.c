...
#endif
    // The row is fixed-height too, 8px taller than its children: content-sized
    // containers compute their height slightly short of the children and clip
    // their bottoms. The buttons are CENTERED in the slack so they line up
    // with the grid-centered progress bar beside them; both heights are fixed,
    // so nothing is computed and nothing clips.
